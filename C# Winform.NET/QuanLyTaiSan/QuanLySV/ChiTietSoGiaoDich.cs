...
namespace QuanLySV
{
    public partial class ChiTietSoGiaoDich : Form
    {
        ...
        public ChiTietSoGiaoDich(string idUser, string idWallet,int loai)
        {
            InitializeComponent();
            IDUser = idUser;
            IDWallet = idWallet;
        ...
        void loadForm(string start, string end)
        {
            ...
            {
                BaoCao fr = new BaoCao(IDUser, IDWallet, start, end) { Dock = DockStyle.Fill, TopLevel = false, TopMost = true };
                panelLoad.Controls.Add(fr);
                fr.FormBorderStyle = FormBorderStyle.None;
                fr.Dock = DockStyle.Fill;
                fr.Show();
