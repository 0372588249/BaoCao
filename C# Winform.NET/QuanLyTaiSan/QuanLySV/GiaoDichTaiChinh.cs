...
namespace QuanLySV
{
    public partial class GiaoDichTaiChinh : Form
    {
        ...
        double getTienNganSach(string id_vi, string id_loai_gd)
        {
            double so_tien = 0;
            string query = @"SELECT so_tien FROM NganSach
            ...
            if(obj != null)
            {
                string strSoTien = obj.ToString();
                so_tien = double.Parse(strSoTien);
            }
        ...
        double getTienDaSuDung(string id_vi, string id_loai_gd)
        {
            string query = @"SELECT dbo.GET_TienNganSach('" + id_vi + "','" + id_loai_gd + "')";
            DB.conn.Open();
            SqlCommand cmd = new SqlCommand(query, DB.conn);
            double so_tien_da_su_dung = double.Parse(cmd.ExecuteScalar().ToString());
            DB.conn.Close();
            return so_tien_da_su_dung;
        }
    }
}
